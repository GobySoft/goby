...
goby::transitional::DCCLMessage::DCCLMessage()
    : size_(0), trigger_number_(1), id_(0), trigger_time_(0.0), repeat_enabled_(false), repeat_(1)
{
    header_.resize(DCCL_NUM_HEADER_PARTS);
    header_[HEAD_CCL_ID] = boost::shared_ptr<DCCLMessageVar>(new DCCLMessageVarCCLID());
    header_[HEAD_DCCL_ID] = boost::shared_ptr<DCCLMessageVar>(new DCCLMessageVarDCCLID());
    header_[HEAD_TIME] = boost::shared_ptr<DCCLMessageVar>(new DCCLMessageVarTime());
    header_[HEAD_SRC_ID] = boost::shared_ptr<DCCLMessageVar>(new DCCLMessageVarSrc());
    header_[HEAD_DEST_ID] = boost::shared_ptr<DCCLMessageVar>(new DCCLMessageVarDest());
    ...
    header_[HEAD_UNUSED] = boost::shared_ptr<DCCLMessageVar>(new DCCLMessageVarUnused());
}
...
void goby::transitional::DCCLMessage::preprocess()
{
    ...
    if (repeat_enabled_)
    {
        glog.is(goby::common::logger::WARN) &&
            glog << "<repeat> is deprecated and ignored in this version of Goby. Simply post a "
...
{
    boost::shared_ptr<acomms::DCCLFieldCodecBase> codec = acomms::DCCLFieldCodecManager::find(
        descriptor_, descriptor_->options().GetExtension(dccl::msg).codec());
    unsigned u = 0;
...
boost::shared_ptr<goby::transitional::DCCLMessageVar>
goby::transitional::DCCLMessage::name2message_var(const std::string& name) const
{
    BOOST_FOREACH (boost::shared_ptr<DCCLMessageVar> mv, layout_)
    {
        if (mv->name() == name)
    ...
    throw goby::acomms::DCCLException(
        std::string("DCCL: no such name \"" + name + "\" found in <layout> or <header>"));
...
void goby::transitional::DCCLMessage::write_schema_to_dccl2(std::ofstream* proto_file)
{
    ...
    *proto_file << "\t"
                << "option (dccl.msg).id = " << id_ << ";" << std::endl;
    *proto_file << "\t"
                << "option (dccl.msg).max_bytes = " << size_ << ";" << std::endl;
