...
// defines a piece of a Message (an <int>, a <bool>, etc)
    class MessageVar
    {
      public:
        ...
        void read_pubsub_vars(std::map<std::string,std::vector<MessageVal> >& vals,
                              const std::map<std::string,std::vector<MessageVal> >& in);
        
        std::string parse_string_val(const std::string& sval);
        
        void var_encode(std::map<std::string,std::vector<MessageVal> >& vals,
                        boost::dynamic_bitset<unsigned char>& bits);
        void var_decode(std::map<std::string,std::vector<MessageVal> >& vals,
                        boost::dynamic_bitset<unsigned char>& bits);

        void set_defaults(std::map<std::string,std::vector<MessageVal> >& vals, unsigned modem_id, unsigned idcd);  
      ...
      protected:
        ...
        virtual void set_defaults_specific(MessageVal& v, unsigned modem_id, unsigned id) { bad_overload("set_defaults_specific()"); }
      ...
      private:
        ...
        void encode_value(const MessageVal& val, boost::dynamic_bitset<unsigned char>& bits);
