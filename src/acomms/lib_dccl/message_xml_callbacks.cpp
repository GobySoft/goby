...
// 
// this is message_xml_callbacks.cpp, part of libdccl
//
    ...
    switch(tags_map_[toNative(localname)])
    {
        ...
        case tag_enum:
            ...
            if((val = attrs.getValue(algorithm.c_str())) != 0)
                messages.back().set_last_algorithms(acomms_util::explode(toNative(val),',',true));
            break;
        ...
        case tag_message_var:
            ...
            {
                if((val = attrs.getValue(algorithm.c_str())) != 0)
                    messages.back().add_last_publish_algorithms(acomms_util::explode(toNative(val),',',true));
                else
