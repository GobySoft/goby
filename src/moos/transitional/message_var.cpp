...
goby::transitional::DCCLMessageVar::DCCLMessageVar()
    : array_length_(1), is_key_frame_(true), sequence_number_(-1), source_set_(false),
...
void goby::transitional::DCCLMessageVar::write_schema_to_dccl2(std::ofstream* proto_file,
                                                               int sequence_number)
{
    ...
    *proto_file << " " << name_ << " = " << sequence_number;
    ...
    try
    {
        ...
        *proto_file << (count ? ", " : " [");
        ++count;
        *proto_file << "(dccl.field).max="
                    << goby::util::as<std::string>(max_tmp, precision(), goby::util::FLOAT_FIXED);
    }
    catch (...)
    {
    }
    try
    {
        ...
        *proto_file << (count ? ", " : " [");
        ++count;
        *proto_file << "(dccl.field).min="
                    << goby::util::as<std::string>(min_tmp, precision(), goby::util::FLOAT_FIXED);
    }
    catch (...)
    {
    }
    ...
    try
    {
        ...
        *proto_file << (count ? ", " : " [");
        ++count;
        *proto_file << "default=\"" << static_val_tmp << "\", (dccl.field).static_value=\""
                    << static_val_tmp << "\", "
    ...
    if (type() == dccl_enum)
    {
        *proto_file << "\t"
        ...
        BOOST_FOREACH (const std::string& e, *enums())
            *proto_file << "\t\t " << boost::to_upper_copy(name_) << "_" << e << " = "
                        << enum_value++ << "; \n";
