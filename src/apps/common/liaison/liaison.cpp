...
goby::common::Liaison::Liaison(protobuf::LiaisonConfig* cfg)
    : ZeroMQApplicationBase(&zeromq_service_, cfg), zeromq_service_(zmq_context_),
      pubsub_node_(&zeromq_service_, cfg->base().pubsub_config())
{
    ...
    try
    {
        ...
        std::string str = cfg->base().app_name() + " --docroot " + doc_root + " --http-port " +
        ...
        wt_server_.addEntryPoint(Wt::Application, goby::common::create_wt_application);
...
void goby::common::Liaison::inbox(MarshallingScheme marshalling_scheme,
                                  const std::string& identifier, const std::string& data,
                                  int socket_id)
{
    glog.is(DEBUG2) && glog << "Liaison: got message with identifier: " << identifier
