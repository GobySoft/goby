...
goby::common::LiaisonScope::LiaisonScope(ZeroMQService* zeromq_service,
    ...
    : LiaisonContainer(parent), MOOSNode(zeromq_service), zeromq_service_(zeromq_service),
      ...
      history_header_div_(
          new HistoryContainer(this, main_layout_, history_model_, moos_scope_config_)),
      controls_div_(new ControlsContainer(&scope_timer_, cfg.start_paused(), this,
                                          subscriptions_div_, history_header_div_)),
      ...
      bottom_fill_(new WContainerWidget)
{
    ...
    zeromq_service_->socket_from_id(LIAISON_INTERNAL_SUBSCRIBE_SOCKET)
        .set_global_blackout(boost::posix_time::milliseconds(1 / cfg.update_freq() * 1e3));
    ...
    scope_tree_view_->sortByColumn(moos_scope_config_.sort_by_column(),
                                   moos_scope_config_.sort_ascending() ? AscendingOrder
                                                                       : DescendingOrder);
    ...
    goby::moos::moos_technique = moos_scope_config_.moos_parser_technique();
                                                ...
                                                CMOOSMsg& msg)
{
    ...
    boost::shared_ptr<google::protobuf::Message> pb_msg = dynamic_parse_for_moos(value);
    ...
    if (pb_msg)
    {
        ...
        items[protobuf::MOOSScopeConfig::COLUMN_TYPE]->setText(
            pb_msg->GetDescriptor()->full_name() + " (Protobuf)");
...
void goby::common::LiaisonScope::update_row(CMOOSMsg& msg, const std::vector<WStandardItem*>& items)
{
    ...
    items[protobuf::MOOSScopeConfig::COLUMN_TIME]->setData(
        WDateTime::fromPosixTime(goby::common::unix_double2ptime(msg.GetTime())), DisplayRole);
...
void goby::common::LiaisonScope::handle_global_key(Wt::WKeyEvent event)
{
    switch (event.key())
    {
        ...
        case Key_Enter:
            ...
            // toggle play/pause
        case Key_P: controls_div_->handle_play_pause(true);
...
void goby::common::LiaisonScope::resume() { controls_div_->resume(); }
...
void goby::common::LiaisonScope::handle_message(CMOOSMsg& msg, bool fresh_message)
{
...
goby::common::LiaisonScopeMOOSTreeView::LiaisonScopeMOOSTreeView(
    const protobuf::MOOSScopeConfig& moos_scope_config, Wt::WContainerWidget* parent /*= 0*/)
    : WTreeView(parent)
{
    this->setAlternatingRowColors(true);

    this->setColumnWidth(protobuf::MOOSScopeConfig::COLUMN_KEY,
                         moos_scope_config.column_width().key_width());
    this->setColumnWidth(protobuf::MOOSScopeConfig::COLUMN_TYPE,
                         moos_scope_config.column_width().type_width());
    this->setColumnWidth(protobuf::MOOSScopeConfig::COLUMN_VALUE,
                         moos_scope_config.column_width().value_width());
    this->setColumnWidth(protobuf::MOOSScopeConfig::COLUMN_TIME,
                         moos_scope_config.column_width().time_width());
    this->setColumnWidth(protobuf::MOOSScopeConfig::COLUMN_COMMUNITY,
                         moos_scope_config.column_width().community_width());
    this->setColumnWidth(protobuf::MOOSScopeConfig::COLUMN_SOURCE,
                         moos_scope_config.column_width().source_width());
    this->setColumnWidth(protobuf::MOOSScopeConfig::COLUMN_SOURCE_AUX,
                         moos_scope_config.column_width().source_aux_width());

    this->resize(Wt::WLength::Auto, moos_scope_config.scope_height());
...
goby::common::LiaisonScopeMOOSModel::LiaisonScopeMOOSModel(
    const protobuf::MOOSScopeConfig& moos_scope_config, Wt::WContainerWidget* parent /*= 0*/)
    : WStandardItemModel(0, protobuf::MOOSScopeConfig::COLUMN_MAX + 1, parent)
{
    ...
    this->setHeaderData(protobuf::MOOSScopeConfig::COLUMN_COMMUNITY, Horizontal,
                        std::string("Community"));
    this->setHeaderData(protobuf::MOOSScopeConfig::COLUMN_SOURCE, Horizontal,
                        std::string("Source"));
    this->setHeaderData(protobuf::MOOSScopeConfig::COLUMN_SOURCE_AUX, Horizontal,
                        std::string("Source Aux"));
}

goby::common::LiaisonScope::ControlsContainer::ControlsContainer(
    Wt::WTimer* timer, bool start_paused, LiaisonScope* scope,
    SubscriptionsContainer* subscriptions_div, HistoryContainer* history_header_div,
    Wt::WContainerWidget* parent /*= 0*/)
    : Wt::WContainerWidget(parent), timer_(timer),
      play_pause_button_(new WPushButton("Play/Pause [p]", this)),
      spacer_(new Wt::WText(" ", this)), play_state_(new Wt::WText(this)), is_paused_(start_paused),
      scope_(scope), subscriptions_div_(subscriptions_div), history_header_div_(history_header_div)
{
...
void goby::common::LiaisonScope::ControlsContainer::pause()
{
    ...
    timer_->stop();
    is_paused_ = true;
    paused_mail_thread_.reset(
        new boost::thread(boost::bind(&ControlsContainer::run_paused_mail, this)));
}
...
goby::common::LiaisonScope::SubscriptionsContainer::SubscriptionsContainer(
    LiaisonScope* node, Wt::WStandardItemModel* model, Wt::WStringListModel* history_model,
    std::map<std::string, int>& msg_map, Wt::WContainerWidget* parent /*= 0*/)
    : WContainerWidget(parent), node_(node), model_(model), history_model_(history_model),
      msg_map_(msg_map), add_text_(new WText("Add subscription (e.g. NAV* or NAV_X): ", this)),
      subscribe_filter_text_(new WLineEdit(this)),
      subscribe_filter_button_(new WPushButton("Apply", this)), subscribe_break_(new WBreak(this)),
      remove_text_(new WText("Subscriptions (click to remove): ", this))
{
    subscribe_filter_button_->clicked().connect(this,
                                                &SubscriptionsContainer::handle_add_subscription);
    subscribe_filter_text_->enterPressed().connect(
        this, &SubscriptionsContainer::handle_add_subscription);
}
...
void goby::common::LiaisonScope::SubscriptionsContainer::add_subscription(std::string type)
{
    ...
    new_button->clicked().connect(
        boost::bind(&SubscriptionsContainer::handle_remove_subscription, this, new_button));
    ...
    const std::string& type)
{
    ...
    for (std::vector<CMOOSMsg>::iterator it = newest.begin(), end = newest.end(); it != end; ++it)
    { node_->handle_message(*it, false); } }

void goby::common::LiaisonScope::SubscriptionsContainer::handle_remove_subscription(
    WPushButton* clicked_button)
{
    ...
    for (int i = model_->rowCount() - 1, n = 0; i >= n; --i)
    {
        ...
        if (remove)
        {
            ...
            for (std::map<std::string, int>::iterator it = msg_map_.begin(), n = msg_map_.end();
                 it != n; ++it)
            {
                if (it->second > i)
                    --it->second;
...
goby::common::LiaisonScope::HistoryContainer::HistoryContainer(
    MOOSNode* node, Wt::WVBoxLayout* main_layout, Wt::WAbstractItemModel* model,
    const protobuf::MOOSScopeConfig& moos_scope_config, Wt::WContainerWidget* parent /* = 0 */)
    : Wt::WContainerWidget(parent), node_(node), main_layout_(main_layout),
      moos_scope_config_(moos_scope_config), hr_(new WText("<hr />", this)),
      add_text_(new WText(("Add history for key: "), this)), history_box_(new WComboBox(this)),
      history_button_(new WPushButton("Add", this)), buffer_(moos_scope_config.max_history_items())
...
void goby::common::LiaisonScope::HistoryContainer::add_history(
    const goby::common::protobuf::MOOSScopeConfig::HistoryConfig& config)
{
    ...
    if (!history_models_.count(selected_key))
    {
        ...
        Wt::WStandardItemModel* new_model =
            new LiaisonScopeMOOSModel(moos_scope_config_, new_container);
        ...
        node_->zeromq_service()
            ->socket_from_id(LIAISON_INTERNAL_SUBSCRIBE_SOCKET)
            .set_blackout(MARSHALLING_MOOS, "CMOOSMsg/" + selected_key + "/",
                          boost::posix_time::milliseconds(0));

        new_proxy->setFilterRegExp(".*");
        new_tree->sortByColumn(protobuf::MOOSScopeConfig::COLUMN_TIME, DescendingOrder);
    }
}
...
{
    for (boost::circular_buffer<CMOOSMsg>::iterator it = buffer_.begin(), end = buffer_.end();
         it != end; ++it)
    { display_message(*it); } buffer_.clear();
}
...
goby::common::LiaisonScope::RegexFilterContainer::RegexFilterContainer(
    ...
    : Wt::WContainerWidget(parent), model_(model), proxy_(proxy), hr_(new WText("<hr />", this)),
      ...
      regex_filter_examples_(new WPushButton("Examples", this)), break_(new WBreak(this)),
      regex_examples_table_(new WTable(this))
{
    ...
    regex_filter_examples_->clicked().connect(this,
                                              &RegexFilterContainer::toggle_regex_examples_table);
    ...
    new WText("-?[[:digit:]]*\\.[[:digit:]]*e?-?[[:digit:]]*",
              regex_examples_table_->elementAt(4, 0));
...
{
    regex_examples_table_->isHidden() ? regex_examples_table_->show()
                                      : regex_examples_table_->hide();
}
