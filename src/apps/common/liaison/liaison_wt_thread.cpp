...
goby::common::LiaisonWtThread::LiaisonWtThread(const Wt::WEnvironment& env) : Wt::WApplication(env)
{
    ...
    else
    {
        ...
        WAnchor* goby_lp_image_a = new WAnchor(
            liaison_cfg_.has_upper_right_logo_link() ? liaison_cfg_.upper_right_logo_link() : "",
    ...
    typedef std::vector<goby::common::LiaisonContainer*> (*liaison_load_func)(
    ...
    for (int i = 0, n = Liaison::plugin_handles_.size(); i < n; ++i)
    {
        liaison_load_func liaison_load_ptr =
            (liaison_load_func)dlsym(Liaison::plugin_handles_[i], "goby_liaison_load");

        if (liaison_load_ptr)
        {
            std::vector<goby::common::LiaisonContainer*> containers =
                (*liaison_load_ptr)(liaison_cfg_, Liaison::zmq_context());
            for (int j = 0, m = containers.size(); j < m; ++j) add_to_menu(menu_, containers[j]);
        }
        else
        {
            glog.is(WARN) &&
                glog << "Liaison: Cannot find function 'goby_liaison_load' in plugin library."
                     << std::endl;
        }
    }
...
void goby::common::LiaisonWtThread::inbox(MarshallingScheme marshalling_scheme,
                                          const std::string& identifier, const void* data, int size,
                                          int socket_id)
{
    glog.is(DEBUG1) && glog << "LiaisonWtThread: got message with identifier: " << identifier
                            << std::endl;
}
