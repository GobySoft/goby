...
void goby::acomms::IridiumShoreDriver::startup(const protobuf::DriverConfig& cfg)
{
    ...
    glog.is(DEBUG1) && glog << group(glog_out_group()) << "Goby Shore Iridium RUDICS/SBD driver starting up." << std::endl;
    ...
    rudics_server_.reset(new RUDICSServer(rudics_io_, driver_cfg_.GetExtension(IridiumShoreDriverConfig::rudics_server_port)));
    mo_sbd_server_.reset(new SBDServer(sbd_io_, driver_cfg_.GetExtension(IridiumShoreDriverConfig::mo_sbd_server_port)));

    rudics_server_->connect_signal.connect(boost::bind(&IridiumShoreDriver::rudics_connect, this, _1));
    
    for(int i = 0, n = driver_cfg_.ExtensionSize(IridiumShoreDriverConfig::modem_id_to_imei); i < n; ++i)
        modem_id_to_imei_[driver_cfg_.GetExtension(IridiumShoreDriverConfig::modem_id_to_imei, i).modem_id()] =
...
void goby::acomms::IridiumShoreDriver::handle_initiate_transmission(const protobuf::ModemTransmission& orig_msg)
{
...
void goby::acomms::IridiumShoreDriver::send(const protobuf::ModemTransmission& msg)
{
    ...
    else if(msg.rate() == RATE_SBD)
    {
        ...
        if(modem_id_to_imei_.count(msg.dest()))
            ...
            glog.is(WARN) && glog << "No IMEI configured for destination address " << msg.dest() << " so unabled to send SBD message." << std::endl;
    }
}

void goby::acomms::IridiumShoreDriver::rudics_send(const std::string& data, goby::acomms::IridiumShoreDriver::ModemId id)
{
    typedef boost::bimap<ModemId, boost::shared_ptr<RUDICSConnection> >::left_map::iterator LeftIt;
    LeftIt client_it = clients_.left.find(id);
    if(client_it != clients_.left.end())
    {
        glog.is(DEBUG1) && glog << "RUDICS sending bytes: " << goby::util::hex_encode(data) << std::endl;
...
void goby::acomms::IridiumShoreDriver::rudics_connect(boost::shared_ptr<RUDICSConnection> connection)
{
    connection->line_signal.connect(boost::bind(&IridiumShoreDriver::rudics_line, this, _1, _2));
    connection->disconnect_signal.connect(boost::bind(&IridiumShoreDriver::rudics_disconnect, this, _1));
}

void goby::acomms::IridiumShoreDriver::rudics_disconnect(boost::shared_ptr<RUDICSConnection> connection)
{
    typedef boost::bimap<ModemId, boost::shared_ptr<RUDICSConnection> >::right_map::iterator RightIt;
    ...
    if(client_it != clients_.right.end())
    {
        ...
        glog.is(DEBUG1) && glog << "Disconnecting client for modem id: " << id << "; " << clients_.size() << " clients remaining." << std::endl;
    }
    else
    {
        glog.is(WARN) && glog << "Disconnection received from connection we do not have in the clients_ map: " << connection->remote_endpoint_str() << std::endl;
    }    
}

void goby::acomms::IridiumShoreDriver::rudics_line(const std::string& data, boost::shared_ptr<RUDICSConnection> connection)
{
    glog.is(DEBUG1) && glog << "RUDICS received bytes: " << goby::util::hex_encode(data) << std::endl;
    ...
    try
    {
        ...
        else if(data == "bye\r")
        {
            ...
            {
                ModemId id = client_it->second;
                glog.is(DEBUG1) && glog << "Detected bye from " <<  connection->remote_endpoint_str() << " ID: " << id << std::endl;
...
void goby::acomms::IridiumShoreDriver::send_sbd_mt(const std::string& bytes, const std::string& imei)
{
    try
    {
        ...
        tcp::resolver::query query(driver_cfg_.GetExtension(IridiumShoreDriverConfig::mt_sbd_server_address), goby::util::as<std::string>(driver_cfg_.GetExtension(IridiumShoreDriverConfig::mt_sbd_server_port)));
...
std::string goby::acomms::IridiumShoreDriver::create_sbd_mt_data_message(const std::string& bytes, const std::string& imei)
{
