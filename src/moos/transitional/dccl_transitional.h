...
class DCCLTransitionalCodec
{
  public:
    ...
    template <typename Key> const google::protobuf::Descriptor* descriptor(const Key& k)
    {
    ...
    template <typename Key> unsigned get_repeat(const Key& k) { return to_iterator(k)->repeat(); }
    ...
    template <typename Key> std::map<std::string, std::string> message_var_names(const Key& k) const
    {
  ...
  private:
    void convert_xml_message_file(
        const goby::transitional::protobuf::MessageFile& message_file, std::string* proto_file,
        google::protobuf::RepeatedPtrField<goby::moos::protobuf::TranslatorEntry>*
            translator_entries,
