...
{
    glog.is(DEBUG1) && glog << "Application destructing..." << std::endl;
}

void goby::pb::Application::__set_up_sockets()
{
    if (!(base_cfg().pubsub_config().has_publish_socket() &&
          base_cfg().pubsub_config().has_subscribe_socket()))
    {
        glog.is(WARN) &&
            glog
                << "Not using publish subscribe config. You will need to set up your nodes manually"
                << std::endl;
    }
    else
    {
        ...
        pubsub_node_.reset(
            new StaticProtobufPubSubNodeWrapper(protobuf_node_.get(), base_cfg().pubsub_config()));
    }
...
{
    glog.is(DEBUG3) && glog << "< [" << group << "]: " << msg << std::endl;
