...
{
  public:
    DCCLMessageVarFloat(double max = std::numeric_limits<double>::max(), double min = 0,
                        double precision = 0);
    ...
    void set_max_delta(double max_delta) { max_delta_ = max_delta; }
    void set_max_delta(const std::string& s) { set_max_delta(util::as<double>(s)); }
  ...
  private:
    bool is_delta() const { return using_delta_differencing() && !is_key_frame_; }

    bool using_delta_differencing() const { return !(boost::math::isnan)(max_delta_); }
