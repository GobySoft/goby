...
void goby::moos::MOOSDBOPlugin::add_message(Wt::Dbo::Session* session, int unique_id, const std::string& identifier, const void* data, int size)
{
    CMOOSMsg msg;
    std::string bytes(static_cast<const char*>(data), size);
    goby::moos::MOOSSerializer::parse(&msg, bytes);
    session->add(new CMOOSMsg(msg));
}

void goby::moos::MOOSDBOPlugin::map_type(Wt::Dbo::Session* session)
{
