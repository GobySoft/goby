...
goby::transitional::DCCLMessageVarFloat::DCCLMessageVarFloat(
    double max /*= std::numeric_limits<double>::max()*/, double min /*= 0*/,
    double precision /*= 0*/)
    : DCCLMessageVar(), max_(max), min_(min), precision_(precision),
...
void goby::transitional::DCCLMessageVarFloat::pre_encode(DCCLMessageVal& v)
{
    double r;
    if (v.get(r))
        v = util::unbiased_round(r, precision_);
}
