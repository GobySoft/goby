...
CpREMUSCodec* CpREMUSCodec::get_instance()
{
    if(!inst_)
        inst_ = new CpREMUSCodec();
    return inst_;
}
      ...
      status_time(10) 
{
    ...
    west = (cfg_.common().lon_origin() < 0.0);
    ...
    subscribe(cfg_.mdat_state_out());
    subscribe(cfg_.mdat_ranger_var());
    subscribe(cfg_.mdat_ranger_out());
    subscribe(cfg_.mdat_redirect_var());
    subscribe(cfg_.mdat_redirect_out());
    subscribe(cfg_.mdat_alert_var());
    subscribe(cfg_.mdat_alert_out());
    subscribe(cfg_.mdat_alert2_var());
    subscribe(cfg_.mdat_alert2_out());
    subscribe("MODEM_ID");
    subscribe("NAV_X");
    subscribe("NAV_Y");
    subscribe("NAV_SPEED");
    subscribe("NAV_DEPTH");
    subscribe("NAV_HEADING");
    subscribe("OUTGOING_COMMAND");
}
CpREMUSCodec::~CpREMUSCodec()
{
}

void CpREMUSCodec::inbox(const CMOOSMsg& msg)
{
    ...
    if(MOOSStrCmp(key, cfg_.mdat_state_var()) || MOOSStrCmp(key, cfg_.mdat_ranger_var())
       || MOOSStrCmp(key, cfg_.mdat_redirect_var()) || MOOSStrCmp(key, cfg_.mdat_alert_var())
       || MOOSStrCmp(key, cfg_.mdat_alert2_var())) // case insensitive
    {
        ...
        string name = modem_lookup_.get_name_from_id(msg_in.base().src());
        string type = modem_lookup_.get_type_from_id(msg_in.base().src());

        string human;
            
        if(MOOSStrCmp(key, cfg_.mdat_state_var()))
        {
            human = decode_mdat_state(bytes, node, name, type);
            m_Comms.Notify("NODE_REPORT", human);
        }
        else if(MOOSStrCmp(key, cfg_.mdat_ranger_var()))
        {
            human = decode_mdat_ranger(bytes, node, name, type);
            m_Comms.Notify("NODE_REPORT", human);
        }
        else if(MOOSStrCmp(key, cfg_.mdat_redirect_var()))
            human = decode_mdat_redirect(bytes, node, name, type);
        else if(MOOSStrCmp(key, cfg_.mdat_alert_var()))
            human = decode_mdat_alert(bytes, node, name, type);
        else if(MOOSStrCmp(key, cfg_.mdat_alert2_var()))
    ...
    {	 
        ...
        if (parseRedirect(msg.m_sVal, dest_id))
        {
            ...
            if (encode_mdat_redirect(bytes))
            {
                ...
                std::cerr << "publishing to " << cfg_.mdat_redirect_out() << " " << out << std::endl;
                m_Comms.Notify(cfg_.mdat_redirect_out(), out);
            }
        }
    } 
} 

void CpREMUSCodec::loop()
{
    double curr_time = MOOSTime();
    bool got_nav = got_x && got_y && got_heading && got_speed && got_depth;

    if (cfg_.create_status() && got_nav && curr_time - status_time  >= status_interval)
    {
        ...
        if (encode_mdat_state(bytes))
	{
            ...
            std::cerr << "publishing to " << cfg_.mdat_state_out() << " " << out << std::endl;

            m_Comms.Notify(cfg_.mdat_state_out(), out);
