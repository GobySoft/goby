...
class Int32RepeatedCodec : public goby::acomms::DCCLRepeatedTypedFieldCodec<goby::int32>
{
  private:
    enum
    {
    ...
    unsigned singular_size() { return goby::util::ceil_log2((max() - min()) + 1); }
    ...
    void validate()
    {
        ...
        DCCLFieldCodecBase::require(
            DCCLFieldCodecBase::dccl_field_options().max_repeat() < MAX_REPEAT_SIZE,
            "(dccl.field).max_repeat must be less than " +
                goby::util::as<std::string>(static_cast<int>(MAX_REPEAT_SIZE)));
    }
};
