...
void goby::common::LiaisonCommander::moos_inbox(CMOOSMsg& msg)
{
    ...
    if (msg.GetKey() == pb_commander_config_.network_ack_var())
    {
        ...
        Dbo::ptr<CommandEntry> acked_command(static_cast<goby::common::CommandEntry*>(0));
        {
            ...
            acked_command = controls_div_->session_.find<CommandEntry>()
                                .where("utime = ?")
                                .bind((long long)ack.message_time());
            if (acked_command)
            {
                glog.is(DEBUG1) && glog << "ACKED command was of type: "
                                        << acked_command->protobuf_name << std::endl;
                ...
                ack_set.SerializeToArray(&acked_command.modify()->acks[0],
                                         acked_command->acks.size());
                transaction.commit();
    ...
    if (display_subscriptions_.count(msg.GetKey()))
    {
        ...
        WGroupBox* box =
            new WGroupBox(msg.GetKey() + " @ " +
                              boost::posix_time::to_simple_string(
                                  goby::util::as<boost::posix_time::ptime>(msg.GetTime())),
                          new_div);
...
goby::common::LiaisonCommander::ControlsContainer::ControlsContainer(
    MOOSNode* moos_node, const protobuf::ProtobufCommanderConfig& pb_commander_config,
    WStackedWidget* commands_div, WContainerWidget* parent /*=0*/)
    : WGroupBox("Controls", parent), moos_node_(moos_node),
      pb_commander_config_(pb_commander_config), command_label_(new WLabel("Message: ", this)),
      command_selection_(new WComboBox(this)), buttons_div_(new WContainerWidget(this)),
...
void goby::common::LiaisonCommander::ControlsContainer::switch_command(int selection_index)
{
    ...
    if (!commands_.count(protobuf_name))
    {
        CommandContainer* new_command =
            new CommandContainer(moos_node_, pb_commander_config_, protobuf_name, &session_);
...
goby::common::LiaisonCommander::ControlsContainer::CommandContainer::CommandContainer(
    MOOSNode* moos_node, const protobuf::ProtobufCommanderConfig& pb_commander_config,
    const std::string& protobuf_name, Dbo::Session* session)
    //    WStackedWidget* master_field_info_stack)
    : WGroupBox(protobuf_name), moos_node_(moos_node),
    ...
    handle_database_double_click(const WModelIndex& index, const WMouseEvent& event)
{
    ...
    edit->clicked().connect(
        boost::bind(&CommandContainer::handle_database_dialog, this, RESPONSE_EDIT, message));
    merge->clicked().connect(
        boost::bind(&CommandContainer::handle_database_dialog, this, RESPONSE_MERGE, message));
    cancel->clicked().connect(
        boost::bind(&CommandContainer::handle_database_dialog, this, RESPONSE_CANCEL, message));
...
void goby::common::LiaisonCommander::ControlsContainer::CommandContainer::handle_database_dialog(
    DatabaseDialogResponse response, boost::shared_ptr<google::protobuf::Message> message)
{
    switch (response)
    {
        case RESPONSE_EDIT:
    ...
    WFormWidget* value_field, const google::protobuf::FieldDescriptor* field_desc)
{
    if (WLineEdit* line_edit = dynamic_cast<WLineEdit*>(value_field))
    {
        ...
        if (pb_commander_config_.has_time_source_var())
        {
            ...
            now = newest.IsDouble() ? unix_double2ptime(newest.GetDouble())
