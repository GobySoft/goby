...
// 
// this is message.cpp, part of libdccl
//
...
void dccl::Message::assemble_hex(micromodem::Message& out_message, const boost::dynamic_bitset<>& bits)
{
    ...
    std::string out = acomms_util::binary_string2hex_string(binary.str());
...
void dccl::Message::disassemble_hex(const micromodem::Message& in_message, boost::dynamic_bitset<>& bits)
{
    ...
    bin_str << acomms_util::hex_string2binary_string(in.substr(bytes2nibs(acomms_util::NUM_HEADER_BYTES)));        
    bin_str >> bits;
}
                                    ...
                                    const std::map<std::string, double>& in_dbl)
{
    ...
    out_message.set_dest(boost::numeric_cast<unsigned int>(acomms_util::sci_round(dval,0)));
}
