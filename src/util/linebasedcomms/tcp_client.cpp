...
bool goby::util::TCPClient::start_specific()
{
    ...
    if(resolver_error)
    {
        glog.is(WARN) && glog << "Error resolving address: " << server_ <<":" << port_ << ": " << resolver_error.message() << std::endl;
        return false;
    }
    ...
    if(error)
        glog.is(WARN) && glog << "Error connecting to address: " << server_ <<":" << port_ << ": " << error.message() << std::endl;
