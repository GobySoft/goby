...
{
    glog.is(DEBUG1, lock) && glog << group(glog_out_group()) << "\tthis is a MICROMODEM_FLEXIBLE_DATA transmission" << std::endl << unlock;
    ...
    if(msg->frame_size() > 0 && msg->frame(0).size())
    {        
        glog.is(DEBUG1, lock) && glog << "FDP data message: " << *msg << std::endl << unlock;
        ...
        if(msg->ack_requested())
            glog.is(WARN, lock) && glog << "ACK not yet supported for FDP" << std::endl << unlock;
    ...
    {
        glog.is(DEBUG1, lock) && glog << group(glog_out_group()) << warn << "FDP transmission failed: no data provided" << std::endl << unlock;
    }
...
void goby::acomms::MMDriver::cardp(const NMEASentence& nmea, protobuf::ModemTransmission* m)
{
    ...
    glog.is(DEBUG1, lock) && glog << group(glog_in_group()) << "Received MICROMODEM_FLEXIBLE_DATA packet from " << m->src() << std::endl << unlock;
