...
void node_inbox(goby::common::MarshallingScheme marshalling_scheme, const std::string& identifier,
...
int main(int argc, char* argv[])
{
    ...
    goby::common::protobuf::ZeroMQServiceConfig publisher_cfg, subscriber_cfg;
    {
        ...
        subscriber_socket->set_connect_or_bind(
...

    {
        ...
        publisher_socket->set_connect_or_bind(
            goby::common::protobuf::ZeroMQServiceConfig::Socket::BIND);
    ...
    // test local blackout
    node2_.socket_from_id(SOCKET_SUBSCRIBE)
        .set_blackout(goby::common::MARSHALLING_CSTR, identifier_,
                      boost::posix_time::milliseconds(6));
    run_basic_test(3, 1, 5);
    run_basic_test(8, 4, 4);

    // remove local blackout
    node2_.socket_from_id(SOCKET_SUBSCRIBE)
        .clear_blackout(goby::common::MARSHALLING_CSTR, identifier_);
...
void run_basic_test(int test_count, int expected_blackouts, int ms_wait)
{
    ...
    for (int i = 0; i < test_count; ++i)
    {
        ...
        node1_.send(goby::common::MARSHALLING_CSTR, identifier_, std::string(data_),
                    SOCKET_PUBLISH);
        node2_.poll(1e6);
