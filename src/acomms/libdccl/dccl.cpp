...
std::string goby::acomms::DCCLCodec::encode(const google::protobuf::Message& msg)
{
    ...
    try
    {
        ...
        boost::shared_ptr<DCCLFieldCodecBase> codec =
            DCCLFieldCodecManager::find(desc, desc->options().GetExtension(dccl_msg).codec());
        ...
        {
            throw(DCCLException("Failed to find (dccl_msg).codec `" + desc->options().GetExtension(dccl_msg).codec() + "`"));
        }
...
boost::shared_ptr<google::protobuf::Message> goby::acomms::DCCLCodec::decode(const std::string& bytes)   
{
    try
    {
        ...
        unsigned head_size_bits = size_id(id), body_size_bits = 0;
        ...
        head_bits.resize(head_size_bits - size_id(id));
...
bool goby::acomms::DCCLCodec::validate(const google::protobuf::Descriptor* desc)
{
    ...
    try
    {
        if(!desc->options().GetExtension(dccl_msg).has_id())
            throw(DCCLException("Missing message option `(dccl_msg).id`. Specify a unique id (e.g. 3) in the body of your .proto message using \"option (dccl.id) = 3\""));
        if(!desc->options().GetExtension(dccl_msg).has_max_bytes())
            throw(DCCLException("Missing message option `(dccl_msg).max_bytes`. Specify a maximum (encoded) message size in bytes (e.g. 32) in the body of your .proto message using \"option (dccl.max_bytes) = 32\""));
        ...
        if(byte_size > desc->options().GetExtension(dccl_msg).max_bytes())
...
void goby::acomms::DCCLCodec::info(const google::protobuf::Descriptor* desc, std::ostream* os)
{
    ...
    {   
        ...
        const unsigned allowed_byte_size = desc->options().GetExtension(dccl_msg).max_bytes();
...
goby::acomms::Bitset goby::acomms::DCCLCodec::encode_id(int id)
{
    if(id <= ONE_BYTE_MAX_ID)
    {
        return(Bitset(size_id(id), id));
    }                
    else
    {
        Bitset return_bits(size_id(id), id);
...
int goby::acomms::DCCLCodec::size_id(int id)
{
...
int goby::acomms::DCCLCodec::decode_id(Bitset bits)
{
    if(bits.test(bits.size()-1))
    {
        // long header
        if(bits.size() > LONG_FORM_ID_BYTES*BITS_IN_BYTE)
        {
            bits >>= bits.size()-LONG_FORM_ID_BYTES*BITS_IN_BYTE;
            bits.resize(LONG_FORM_ID_BYTES*BITS_IN_BYTE);
        }
    ...
    {
        // short header
        if(bits.size() > SHORT_FORM_ID_BYTES*BITS_IN_BYTE)
        {
            bits >>= bits.size()-SHORT_FORM_ID_BYTES*BITS_IN_BYTE;
            bits.resize(SHORT_FORM_ID_BYTES*BITS_IN_BYTE);
        }
        return bits.to_ulong();
    }   
}
