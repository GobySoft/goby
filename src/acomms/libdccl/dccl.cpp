...
boost::shared_ptr<google::protobuf::Message> goby::acomms::DCCLCodec::decode(const std::string& bytes)   
{
    try
    {
        ...
        boost::shared_ptr<google::protobuf::Message> msg = 
            goby::core::DynamicProtobufManager::new_protobuf_message(id2desc_.find(id)->second);
        ...
        unsigned head_size_bits = codec->base_max_size(desc, DCCLFieldCodecBase::HEAD) + fixed_head_size();
        unsigned body_size_bits = codec->base_max_size(desc, DCCLFieldCodecBase::BODY);

        unsigned head_size_bytes = ceil_bits2bytes(head_size_bits);
        unsigned body_size_bytes = ceil_bits2bytes(body_size_bits);
    
        glog.is(debug1) && glog  << "head is " << head_size_bits << " bits" << std::endl;
        glog.is(debug1) && glog  << "body is " << body_size_bits << " bits" << std::endl;
        glog.is(debug1) && glog  << "head is " << head_size_bytes << " bytes" << std::endl;
        glog.is(debug1) && glog  << "body is " << body_size_bytes << " bytes" << std::endl;
        ...
        glog.is(debug1) && glog  << "head is " << hex_encode(head_bytes) << std::endl;
        glog.is(debug1) && glog  << "body is " << hex_encode(body_bytes) << std::endl;
...
bool goby::acomms::DCCLCodec::validate(const google::protobuf::Descriptor* desc)
{
    ...
    try
    {
        ...
        const unsigned head_bit_size = codec->base_max_size(desc, DCCLFieldCodecBase::HEAD) + fixed_head_size();
        const unsigned body_bit_size = codec->base_max_size(desc, DCCLFieldCodecBase::BODY);
...
void goby::acomms::DCCLCodec::info(const google::protobuf::Descriptor* desc, std::ostream* os)
{
    ...
    {   
        ...
        const unsigned config_head_bit_size = codec->base_max_size(desc, DCCLFieldCodecBase::HEAD);
        const unsigned body_bit_size = codec->base_max_size(desc, DCCLFieldCodecBase::BODY);
        ...
        *os << "= Header =" << std::endl;
        codec->base_info(desc, os, DCCLFieldCodecBase::HEAD);
        *os << "= Body =" << std::endl;
        codec->base_info(desc, os, DCCLFieldCodecBase::BODY);
