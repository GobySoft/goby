// t. schneider tes@mit.edu 05.18.09
// ocean engineering graudate student - mit / whoi joint program
// massachusetts institute of technology (mit)
// laboratory for autonomous marine sensing systems (lamss)
// 
// this is modem_message.h
//
    ...
    class Message
    {
      ...
      Message(const std::string & s = ""):
        ...
        std::string snip() const 
        {
            ...
            if(t_set_)     ss << " | age " << acomms_util::sci_round(time(NULL) - t_, 0) << "s";
        ...
        void unserialize(const std::string & s) 
        {
            ...
            {
                if(acomms_util::val_from_string(value, lower_s, "src"))
                    set_src(value);
                if(acomms_util::val_from_string(value, lower_s, "dest"))
                    set_dest(value);
                if((acomms_util::val_from_string(value, lower_s, "data") || acomms_util::val_from_string(value, lower_s, "hexdata")) && check_hex(value))
                    set_data(value);
                if(acomms_util::val_from_string(value, lower_s, "time"))
                    set_t(value);
                if(acomms_util::val_from_string(value, lower_s, "ack"))
                    set_ack(value);
                if(acomms_util::val_from_string(value, lower_s, "frame"))
                    set_frame(value);
            }
        ...
        {
            try { set_ack(acomms_util::string2bool(ack)); }
            catch(boost::bad_lexical_cast & ) { }
        }
