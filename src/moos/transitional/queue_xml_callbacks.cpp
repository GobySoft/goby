    ...
    const XMLCh* const qname)     // tagname + NS pefix
{
    ...
    switch (current_tag)
    {
        ...
        case tag_ack: q_.back().set_ack(as<bool>(trimmed_data)); break;

        case tag_blackout_time: q_.back().set_blackout_time(as<uint32>(trimmed_data)); break;

        case tag_max_queue: q_.back().set_max_queue(as<uint32>(trimmed_data)); break;

        case tag_newest_first: q_.back().set_newest_first(as<bool>(trimmed_data)); break;

        case tag_id: q_.back().mutable_key()->set_id(as<uint32>(trimmed_data)); break;
        ...
        case tag_ttl: q_.back().set_ttl(as<uint32>(trimmed_data)); break;

        case tag_value_base: q_.back().set_value_base(as<double>(trimmed_data)); break;
    }
}
